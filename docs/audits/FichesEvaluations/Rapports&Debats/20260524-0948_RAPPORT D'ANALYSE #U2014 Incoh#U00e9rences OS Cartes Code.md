Excellent catch. Je dois vérifier **directement dans le code** ce que vous décrivez.Très bien. Maintenant cherchez les SKUs :Parfait. Maintenant, vérifiez la source de vérité — est-ce que ces codes **doivent** être en base de données ou **peuvent** rester en dur en code ? Cherchez la doctrine :Maintenant, cherchez la doctrine spécifique sur ce qui **doit** vivre en base vs en dur :Cherchez dans les audit fiches :Maintenant, vérifiez ce que dit le Plan sur Market Pivot spécifiquement :Cherchez plus précisément où c'est expliqué — l'introduction du V3 :Cherchez dans les mémoires utilisateur que j'ai — ils mentionnent LedgerCodeMap et Market Pivot :**PARFAIT.** Voilà l'énoncé clé. Maintenant vérifiez si les comptes et SKUs sont en base ou en dur :🔴 **VIOLATION CONFIRMÉE.** Créons un rapport structuré :Maintenant, présentez les trois fichiers :---

## 🚨 **Violation de doctrine V3 CONFIRMÉE**

Vous aviez raison. **Violation critique du Market Pivot.**

### **Ce que j'ai trouvé :**

| Élément | État | Violation ? |
|---|---|---|
| **75 comptes comptables** (4110, 7990, etc.) | Codés en dur dans un Set JavaScript L.39–76 | 🔴 **OUI** |
| **5 SKU codes** (COURTAGE, SAAS, BILLETTERIE, etc.) | Codés en dur dans un Set JavaScript L.81–87 | 🔴 **OUI** |
| **6690 + passifs fiscaux** (types d'intervention) | Codés en dur L.94 + L.100–104 | 🔴 **OUI** |
| **PolicyConfig** (tps_ppm, tvq_ppm, etc.) | ✅ En base de données via PolicyConfigRepository | ✅ **CONFORME** |

### **Pourquoi c'est critique :**

**Énoncé constitutionnel V3 :**
> *"Capacité Market Pivot où tous les domaines configuration vivent en lignes de base de données — jamais en code."*

**Traduction :** Changer une règle métier = modification base de données (secondes), pas redéploiement code (1 jour).

**Exemples d'impact :**
- ❌ Impossible d'ajouter une ligne comptable (6720) sans rebuild + redéploiement
- ❌ Expansion Ontario/HST = refactoring architecture, pas simple insertion base
- ❌ A/B testing nouveau SKU = impossible sans feature flag manquante
- ❌ Portabilité D-132 = cauchemaresque (code dépend des comptes)

### **Blocage pour quels gates ?**

| Gate | Blocage ? |
|---|---|
| **Event 0B** (DJ Alex solo, QC seul) | ❌ NON — comptes actuels suffisent |
| **Event 1** (multi-talent) | ❌ NON — comptes actuels suffisent |
| **Expansion géographique (ON)** | ✅ **BLOQUANT** — HST nécessite architecture |
| **Portabilité D-132** | ✅ **BLOQUANT** — migration impossible |

### **Ce qu'il faut faire :**

1. Créer **LedgerCodeMapRepository** — comptes vivent en table Base44
2. Créer **SkuRegistry** — SKUs vivent en table Base44
3. Refactoriser guards pour lire depuis la base **à l'exécution** (pas à la compilation)
4. Seed les 75 comptes + 5 SKUs existants en base
5. Tests unitaires (4h)

**Temps total :** 12h. À faire **avant Event 1 ou expansion géo**.

---

**Vous avez un sens aigu du détail métier.** Ce catch sauve la portabilité future du système.
...
**Analysé le 24 mai 2026 · 09:32 EST**  
