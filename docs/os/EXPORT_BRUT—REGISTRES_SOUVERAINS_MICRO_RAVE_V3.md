...
**Date de génération :** 13 mai 2026 **Portée :** BLOCs 0 à 18 — interrogatoire procédural complet **Total des décisions :** 150 décisions validées
...
deposit\_secured → balance\_pending / transfer\_requested / cancelled\_J30  
transfer\_requested → transfer\_accepted / transfer\_refused / no\_show\_pre\_event  
balance\_pending → event\_sealed / cancelled\_J7  
event\_sealed → performed  
performed → event\_completed / disputed  
event\_completed → sots\_window\_closed  
sots\_window\_closed → payable / disputed / no\_show  
payable → settled  
settled → archived  
disputed → payable / partially\_settled / refunded  
no\_show → refunded → archived  
refunded → archived  
